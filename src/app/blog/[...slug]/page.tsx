...
export async function generateMetadata({
  ...
  const post = await getBlogPost(slugKey)
...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  const slugTail = segments[segments.length - 1] ?? slug
      ...
      <div className="container-custom max-w-4xl mb-12">
        <div className="flex items-center gap-2 text-sm font-mono text-muted-foreground mb-8 overflow-x-auto whitespace-nowrap">
          ...
          {post.section ? (
            <>
              <Link
                href={`/blog/folder/${encodeURIComponent(post.section)}`}
          ...
          <ChevronRight className="w-3 h-3 shrink-0" />
          <span className="text-foreground truncate max-w-[min(40vw,12rem)]">
            {slugTail}
          </span>
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {relatedPosts.map((relatedPost) => (
              <Link
                ...
                href={postPathFromSlug(relatedPost.slug)}
