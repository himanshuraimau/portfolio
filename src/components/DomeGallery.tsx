...
import { useEffect, useMemo, useRef, useCallback, CSSProperties, MouseEvent, PointerEvent } from 'react';
...
function buildItems(pool: (DomeGalleryImage | string)[], seg: number): GridItem[] {
  ...
  const normalizedImages = pool.map(image => {
    if (typeof image === 'string') {
      return { src: image, thumbnailSrc: image, alt: '' };
    }
    return {
      src: image.src || '',
  ...
  }).filter(img => img.src && img.src.trim() !== '' && !img.src.includes('undefined'));
  ...
  return coords.map((c, i) => ({
    ...
    thumbnailSrc: usedImages[i].thumbnailSrc,
    alt: usedImages[i].alt
  }));
}

function computeItemBaseRotation(offsetX: number, offsetY: number, sizeX: number, sizeY: number, segments: number): ItemRotation {
...
export default function DomeGallery({
  ...
  const startRotRef = useRef<Position>({ x: 0, y: 0 });
  ...
  const applyTransform = (xDeg: number, yDeg: number): void => {
  ...
  useGesture(
    {
      onDragStart: ({ event }) => {
        ...
        stopInertia();
        const evt = event as globalThis.MouseEvent | globalThis.PointerEvent;
  ...
  const openItemFromElement = useCallback(
    (el: HTMLDivElement) => {
      ...
      lockScroll();
      const parent = el.parentElement as HTMLElement | null;
      if (!parent) return; // Should not happen given structure
                ...
                >
                  <img
                    ...
                    onError={(e) => {
                      ...
                      // Also hide parent container to avoid empty black boxes
