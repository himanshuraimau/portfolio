...
export function SkillsToggle() {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto border border-border rounded-xl bg-card overflow-hidden shadow-sm">
      ...
      {/* Category Toggle Bar - Improved Responsive Design */}
      <div className="w-full overflow-x-auto border-b border-border bg-muted/20 scrollbar-thin scrollbar-thumb-muted scrollbar-track-transparent">
        <div className="flex min-w-max md:min-w-0 md:justify-stretch">
          {[
            ...
            return (
              <button
                ...
                className={cn(
                  "flex items-center justify-center gap-1.5 px-3 sm:px-4 md:flex-1 py-2.5 sm:py-3 text-xs sm:text-sm font-mono transition-all border-r border-border last:border-r-0 whitespace-nowrap",
      ...
      {/* Skills Grid Area - Fixed Text Wrapping */}
      <div className="p-4 sm:p-6 md:p-8 bg-background/50 min-h-[250px] sm:min-h-[280px] md:min-h-[300px]">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-3 md:gap-4"
          >
            {skillsData[activeCategory].map((skill, index) => (
              <motion.div
                ...
                className="group relative border border-border bg-card/50 hover:bg-muted/30 p-3 sm:p-3.5 rounded-lg flex items-center justify-between transition-colors"
              >
                <div className="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1 overflow-hidden">
                  ...
                  <span className="font-medium font-mono text-xs sm:text-sm text-foreground whitespace-nowrap overflow-hidden text-ellipsis">{skill}</span>
                </div>

                <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-green-500/50 group-hover:bg-green-500 group-hover:shadow-[0_0_8px_rgba(34,197,94,0.6)] transition-all shrink-0 ml-2" />
              </motion.div>
            ))}
          </motion.div>
        </AnimatePresence>
