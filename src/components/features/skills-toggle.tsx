  ...
  return (
    <div className="w-full border border-border rounded-xl bg-card overflow-hidden shadow-sm">
      ...
      <div className="w-full overflow-x-auto border-b border-border bg-muted/20 scrollbar-thin scrollbar-thumb-muted scrollbar-track-transparent">
        <div className="flex min-w-max lg:min-w-0 lg:justify-stretch">
          {[
      ...
      <div className="p-4 md:p-6 bg-background/50 min-h-[250px]">
          ...
          >
            {skillsData[activeCategory].map((skill, index) => (
              <motion.div
                ...
                className="group relative border border-border bg-card/50 hover:bg-muted/30 p-3 rounded-lg flex items-center justify-between transition-colors"
              >
                <div className="flex items-center gap-2.5 min-w-0 flex-1">
                  <span className="text-xs font-mono text-muted-foreground/50 group-hover:text-primary transition-colors shrink-0">
                    {(index + 1).toString().padStart(2, "0")}
                  </span>
                  <span className="font-medium font-mono text-sm text-foreground truncate">{skill}</span>
                </div>

                <div className="w-2 h-2 rounded-full bg-green-500/50 group-hover:bg-green-500 group-hover:shadow-[0_0_8px_rgba(34,197,94,0.6)] transition-all shrink-0 ml-2" />
              </motion.div>
            ))}
          </motion.div>
        </AnimatePresence>
